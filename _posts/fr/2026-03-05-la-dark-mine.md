...
header_image: "/assets/images/dark-mine.png"
...
La différence est structurelle. Dans une dark factory, le processus est borné : on sait ce qui entre, on sait ce qui sort, on optimise le chemin entre les deux. Le code suit bien cette logique : un ticket, un spec et un livrable. L'agent remplace le développeur sur des tâches définies, et le gain se mesure en vélocité.
...
C'est cette logique qui me sert de baseline dans le contexte Jolimoi. Aujourd'hui, un Data Analyst répond à des questions qu'un humain a déjà formulées. Le biais est structurel : on ne peut trouver que ce qu'on cherche. Une dark mine génère des questions que personne n'aurait posées, et c'est fondamentalement différent. La découverte naît de l'hypothèse que personne n'avait jugé utile de tester, ni même imaginé.
...
Le risque n'est pas que la mine ne trouve rien. C'est qu'elle trouve tout, et que l'organisation se noie dans un minerai qu'elle ne sait pas raffiner.
