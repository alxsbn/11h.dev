...
## No colon-then-fragment

Do not use a colon to introduce a dangling fragment. If the text after the colon is not a full clause or a proper list, restructure as a complete sentence.

| Don't write | Write instead |
|-------------|---------------|
| l'outil est plus intime : un agent à qui on parle. | l'outil est plus intime. Nous sommes face à un agent à qui on parle. |
| with one difference: no correction mechanism. | with one notable difference: no correction mechanism is built in. |
